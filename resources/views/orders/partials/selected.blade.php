                ...
                <div class="btn-group btn-group-justified" data-toggle="buttons">                
                    @if($order->gender != null)
                        <label class="btn blue {{($order->gender=='M')? 'active': ''}}">
                            <input data-tab="gender" aria-hidden="true" name="selected[gender]" class="styled-checkbox selected toggle" type="radio" value="M" {{ $order->gender == 'M' ? 'checked' : '' }}><i class="fa fa-male" aria-hidden="true"></i> @lang('Man') 
                        </label>
                        <label class="btn blue {{($order->gender=='K')? 'active': ''}}">                    
                            <input data-tab="gender" aria-hidden="true" name="selected[gender]" class="styled-checkbox selected toggle" type="radio" value="K" {{ $order->gender == 'K' ? 'checked' : '' }}><i class="fa fa-female" aria-hidden="true"></i> @lang('Woman') 
                        </label>
                        <label class="btn blue {{($order->gender=='0')? 'active': ''}}">                    
                            <input data-tab="gender" name="selected[gender]" class="styled-checkbox selected toggle" type="radio" value="0" {{ $order->gender == '0' ? 'checked' : '' }}><i class="fa fa-users"></i> Both 
                        </label>
                    @else                    
            ...
            <div class="col-md-8">
                <div id="spinner4">
                    <div class="input-group">
                        ...
                        <input data-tab="age" type="number" class="input-md form-control selected spinner-input form-control" name="selected[age][from]" value="{{ $order->age['from'] }}"/>
                        <div class="spinner-buttons input-group-btn">
            ...
            <div class="col-md-8">
                <div id="spinner3">
                    <div class="input-group">
                        ...
                        <input data-tab="age" type="number" class="input-md form-control selected spinner-input form-control" name="selected[age][to]" value="{{ $order->age['to'] }}"/>
                        <div class="spinner-buttons input-group-btn">
                            <button type="button" class="btn spinner-down red">
                            <i class="fa fa-minus"></i>
                            </button>
                                        ...
                                        @if(is_array($order->geography['county']))
                                            @foreach($order->geography['county'] as $key => $val)
                                                <li class="tab_sublist_item">
                                                    ...
                                                    <label for="styled-checkbox" class="category_sub_title">
                                                        <span> <i class="fas fa fa-angle-down"></i>{{ $key }}</span>
                                                    </label>
                                                    <ul class="tab_sublist">
                                                        @if(is_array($val))
                                        ...
                                        @if(is_array($order->geography['zip']))
                                            @foreach($order->geography['zip'] as $key => $val)
                                                <li class="tab_sublist_item">
                                                    ...
                                                    <label for="styled-checkbox" class="category_sub_title">
                                                        <span> <i class="fas fa fa-angle-down"></i>{{ $val }}</span>
                                                    </label>
                                                </li>
                                            @endforeach
                                        @endif
                ...
                <div class="form-group last">
                    <div class="col-md-12">
                        <div class="md-checkbox para-color">                
                            <input data-tab="living" name="selected[living_type][{{ $key }}]" class="styled-checkbox selected md-check" type="checkbox" value="1" id="checkbox3{{$key}}" {{ isset($order->living_type[$key]) ? 'checked' : '' }}>
                    ...
                    @if($order->phone_numbers)
                        @foreach(\App\Order::$phone_numbers as $key => $val)
                            <div class="md-radio para-color">                            
                                <input data-tab="phone" name="selected[phone_numbers]" class="styled-checkbox selected md-radiobtn" type="radio" value="{{ $key }}" id="radio$key" {{ $order->phone_numbers == $key ? 'checked' : '' }}>
                                <label for="radio9">
                                    {{ __($val) }} 
                                </label>
                            </div>
